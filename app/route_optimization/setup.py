...
def setup_osmnx(city, north, south, east, west,
                     ...
                     target_list,
                     dead_ends=True):
    G = fetch_city(city, north, south, east, west)
    ...
    carriers, transportables, targets = set_objects(G, carrier_list, transportable_list, target_list)
    ...
    dic = find_first_paths(G, carriers, transportables)
    inter_dic = find_inter_paths(G, transportables, targets, len(carriers))
    target_dic = find_trans_paths(G, transportables, targets)
