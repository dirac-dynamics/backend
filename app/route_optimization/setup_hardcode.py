...
def setup_osmnx(city, north, south, east, west,
    ...
    for i in range(1,11):
        for j in range(1,11):
            all_route_index.append([i,j])
    
    all_coord_routes, all_time, all_length = uo.output_routes(G,all_route_index,dic)

    return carriers, targets, transportables, all_coord_routes, all_time, all_length, dic['weight_list'], dic['connection_list_single'], dic['connection_number'], dic['weight_list_2'], dic['start_end_list']
