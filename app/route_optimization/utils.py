...
def fetch_city(city):
    if isinstance(city, str):
        data = json.load(open(maps_path + '/places.json'))
        if city in data:
            ...
            ox.io.save_graphml(graph, filepath=maps_path + '/%s.graphml' % (city))
            data[city] = maps_path + '/%s.graphml' % (city)
            with open(maps_path + '/places.json', 'w') as outfile:
...
def fetch_city_2(name,north,south,east,west):
    if isinstance(name, str):
        ...
        if name in data:
            ...
            ox.io.save_graphml(graph, filepath=maps_path + '/%s.graphml' % (name))
            data[name] = maps_path + '/%s.graphml' % (name)
            with open(maps_path + '/places.json', 'w') as outfile:
                json.dump(data, outfile)
